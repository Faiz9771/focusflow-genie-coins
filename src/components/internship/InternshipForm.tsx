...
export default function InternshipForm({ onSuccess, onCancel }: InternshipFormProps) {
  ...
  async function onSubmit(values: InternshipFormValues) {
    try {
      ...
      if (!session?.user) {
        toast.error("You must be logged in to add an internship application");
        return;
      }
      ...
      toast.error("Failed to save internship application");
    } finally {
